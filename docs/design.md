...
**slugify():** タイトル全体の SHA-256 先頭6文字をハッシュ接尾辞として必ず付与し、純日本語タイトルの同一パス衝突を防ぐ（例: `"Docker Setup Guide"` → `"docker-setup-guide-a3f9c1"` / `"Docker起動手順"` → `"docker-a3f9c1"` / `"起動手順"` → `"a3f9c1"`）。kebab-case のみ有効。ダブルハイフン `--` は発生しない。
...
### §8.1: SKILL.md 読み込み仕様（確定・公式ガイド準拠）

**自動スキャンで確定（仮説A）。** Claude Code は `~/.claude/skills/` および `.claude/skills/` 配下のディレクトリを自動スキャンし、各 `SKILL.md` の YAML frontmatter をシステムプロンプトに含める。追加の設定変更は不要。

**3層 Progressive Disclosure:**

| 層 | 内容 | ロードタイミング |
|----|------|----------------|
| Level 1 | YAML frontmatter（name, description） | 常にシステムプロンプトにロード |
| Level 2 | SKILL.md 本文（手順・例・トラブルシューティング） | スキルが関連すると判断された時 |
| Level 3 | `references/` 配下のファイル | Claude が必要と判断した時のみ |

**YAML frontmatter 必須フィールド:**

```yaml
---
name: skill-name-in-kebab-case   # フォルダ名と一致・kebab-case のみ
description: |
  [何をするか]. Use when user says "[トリガーフレーズ1]",
  "[トリガーフレーズ2]", or "[トリガーフレーズ3]".
metadata:                        # 任意
...
**制約:**
- `name`: kebab-case のみ（スペース・大文字・アンダースコア禁止）。フォルダ名と一致
- `description`: WHAT（何をするか）+ WHEN（トリガーフレーズ）の両方必須。1024文字以内
- XML タグ `< >` はフロントマター内で禁止（セキュリティ制約）
- SKILL.md 本文は 5,000 ワード以内。詳細は `references/` に分離
- `"claude"` `"anthropic"` を name に含めることは禁止（予約語）

**生成物のディレクトリ構造:**

```
{SKILLS_OUTPUT_DIR}/{slug}/
├── SKILL.md          # frontmatter + 手順（5,000ワード以内）
└── references/       # 詳細ドキュメント（必要時のみロード）
```
